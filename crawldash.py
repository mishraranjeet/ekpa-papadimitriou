...
def crawl247(pages=pages247, agent=agent):
    ...
    mergedUpdates = (
        pd.concat([newsUpdate, df247]).drop_duplicates(subset=["title"], keep="last")
    ).reset_index(drop=True)
    mergedUpdates["time"] = pd.to_datetime(mergedUpdates["time"])
    mergedUpdates = mergedUpdates.sort_values(by="time", ascending=False).reset_index(
        drop=True
    )
...
def crawlcapital(pages=pagescap, agent=agent):
    ...
    mergedUpdates = (
        pd.concat([newsUpdate, capital]).drop_duplicates(subset=["title"], keep="last")
    ).reset_index(drop=True)
    mergedUpdates["time"] = pd.to_datetime(mergedUpdates["time"])
    mergedUpdates = mergedUpdates.sort_values(by="time", ascending=False).reset_index(
        drop=True
...
def crawliefimerida(pages=pagesiefim):
    ...
    mergedUpdates = (
        pd.concat([newsUpdate, iefimerida]).drop_duplicates(
...
def vectorization(df247, capital, iefimerida):
    ...
    tfidf_vectorizer = TfidfVectorizer(
        stop_words=greek_stopwords, max_features=500, min_df=0.01, max_df=0.95
    )
    ...
    for i in range(0, len(arr2.columns)):
        similar_titles.iloc[i, :6] = (
            arr2.iloc[0:, i].sort_values(ascending=False)[:6].index
        )
    ...
    frontDf1 = frontDf1.merge(
        dfcountry, how="inner", left_on=["countrycode"], right_on=["2let"]
    )
    frontDf1.drop(
        frontDf1.columns.difference(
            ["title", "text", "time", "image", "country", "3let"]
        ),
        1,
        inplace=True,
    )
    ...
    mergedUpdates = (
        pd.concat([newsUpdate, frontDf1]).drop_duplicates(subset=["title"], keep="last")
    ).reset_index(drop=True)
    mergedUpdates["time"] = pd.to_datetime(mergedUpdates["time"])
    mergedUpdates = mergedUpdates.sort_values(by="time", ascending=False).reset_index(
        drop=True
